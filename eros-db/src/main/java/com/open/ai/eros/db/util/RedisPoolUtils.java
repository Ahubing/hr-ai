...
public class RedisPoolUtils {
    ...
    public RedisPoolUtils(RedisProperties redisConfig) {
        try {
            JedisPoolConfig config = new JedisPoolConfig();

            RedisProperties.Pool jedisPoolConfig = redisConfig.getJedis().getPool();
            ...
            //最大连接数, 默认8个
            config.setMaxTotal(jedisPoolConfig.getMaxIdle());
            //获取连接时的最大等待毫秒数(如果设置为阻塞时BlockWhenExhausted),如果超时就抛异常, 小于零:阻塞不确定的时间,  默认-1
            config.setMaxWaitMillis(jedisPoolConfig.getMaxWait().toMillis());
