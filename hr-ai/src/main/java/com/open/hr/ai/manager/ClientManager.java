    ...
    public ResultVO finishClientTask(String bossId, String connectId, ClientFinishTaskReq clientFinishTaskReq) {
        ...
        try {
            ...
            switch (taskType){
                ...
                case "request_all_info": dealUserAllInfoData(taskId,amZpLocalAccouts,data); break;
                case "switch_job_state": switchJobState(taskId,bossId,data); break;
                default: log.error("找不到该任务 req={}",clientFinishTaskReq); break;
            }
            return ResultVO.success();
        } catch (Exception e) {
        ...
        for (int i = 0; i < resumes.size(); i++) {
            try {
                ...
                if (Objects.isNull(amResume)){
                    log.error("greetHandle amResume is null,bossId={},resume={}",bossId,resumes.get(i));
                    continue;
                }
                //查看账号是否开启打招呼
