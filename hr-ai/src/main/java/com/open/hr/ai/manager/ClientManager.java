...
public class ClientManager {
    ...
    public ResultVO bossNewMessage(String platform,String bossId, String connectId, ClientBossNewMessageReq req) {
        ...
        try {
            ...
            List<AmChatbotGreetResult> amChatbotGreetResults = amChatbotGreetResultService.list(new QueryWrapper<AmChatbotGreetResult>().eq("account_id", bossId).eq("user_id", req.getUser_id()).eq("success", 1));
            if (CollectionUtils.isNotEmpty(amChatbotGreetResults)) {
                // 批量更新
                List<Integer> ids = amChatbotGreetResults.stream().map(AmChatbotGreetResult::getId).collect(Collectors.toList());
                if (CollectionUtils.isNotEmpty(ids)) {
                    LambdaUpdateWrapper<AmChatbotGreetResult> updateWrapper = new LambdaUpdateWrapper<>();
                    updateWrapper.in(AmChatbotGreetResult::getId, ids);
                    updateWrapper.set(AmChatbotGreetResult::getSuccess, 2);
                    boolean updated = amChatbotGreetResultService.update(updateWrapper);
                    log.info("批量更新成功,AmChatbotGreetResult ids={}, updated={}", ids, updated);
                }
            }
        ...
        try {
            ...
            if (success) {
                ...
                if (tasksServiceOne.getRetryTimes() < 3 && !clientFinishTaskReq.getNot_retry()) {
                    tasksServiceOne.setDetail(tasksServiceOne.getDetail()+"\n"+reason);
                ...
                dealErrorTask(taskId,amZpLocalAccouts, tasksServiceOne);
            }
...
//                    }
    ...
    private void greetHandle(String platform, AmClientTasks tasksServiceOne, String taskId, String bossId, JSONObject finishTaskReqData) {
        try {
            ...
            String detail = tasksServiceOne.getDetail();
            ...
            for (int i = 0; i < resumes.size(); i++) {
                ...
                try {
                    ...
                    Boolean filterAmResumeResult = innerFilterAmResume(chatbotGreetConditionNew, amResume, false,filterReason);
                    ...
                    if (Objects.isNull(one) || one.getIsGreetOn() == 0) {
                        stringBuilder.append("\n").append(amZpLocalAccouts.getAccount()).append("未开启打招呼配置,").append(amResume.getName()).append("不进行后续流程");
//                        tasksServiceOne.setDetail(stringBuilder.toString());
//                        amClientTasksService.updateById(tasksServiceOne);
                    ...
                    if (Objects.isNull(amChatbotPositionOption)) {
                        log.info("复聊任务处理异常, 账号:{}, 未找到对应的职位", amZpLocalAccouts.getId());
                        stringBuilder.append("\n"+"复聊任务处理异常, 账号:"+amZpLocalAccouts.getId()+", 未找到对应的职位");
                        continue;
                    }
                    // 查询第一天的复聊任务
                    ...
                    if (Objects.isNull(amChatbotOptionsItems) || amChatbotOptionsItems.isEmpty()) {
                        log.info("复聊任务处理异常, 账号:{}, 未找到对应的复聊方案", amZpLocalAccouts.getId());
                        stringBuilder.append("\n"+"复聊任务处理异常, 账号:"+amZpLocalAccouts.getId()+", 未找到对应的复聊方案");
                        continue;
                    }
                    ...
                    for (AmChatbotOptionsItems amChatbotOptionsItem : amChatbotOptionsItems) {
                        ...
                        if (!result) {
                            log.error("greetHandle amChatbotGreetResultService save error,bossId={},amChatbotGreetResult={}", bossId, amChatbotGreetResult);
                            continue;
                        }
                        ...
                        log.info("打招呼定时复聊任务处理开始, 账号:{}, 复聊任务:{},添加结果:{}", amZpLocalAccouts.getId(),amChatbotGreetResult, zadd);
                    }
                } catch (Exception e) {
    ...
    private void dealUserAllInfoData(String platform, String taskId, AmZpLocalAccouts amZpLocalAccouts, JSONObject jsonObject) {
        ...
        try {
            ...
            String userId = chatInfoJSONObject.get("uid").toString();
            if (StringUtils.isNotBlank(userId)) {
                ...
                if (Objects.isNull(amResume)) {
                    ...
                    if (Objects.nonNull(resumeJSONObject)){
                        JSONObject searchData = resumeJSONObject.getJSONObject("search_data");
                        requestInfoDealAmResume(amResume, resumeJSONObject, searchData);
                    }
...
//        amResume.setSalary(Objects.nonNull(geekBaseInfo.get("salaryDesc")) ? geekBaseInfo.get("salaryDesc").toString() : "");
    ...
    private AmResume requestInfoDealAmResume(AmResume amResume, JSONObject resumeJSONObject, JSONObject searchData) {
    ...
    private void dealErrorTask(String taskId, AmZpLocalAccouts amZpLocalAccouts, AmClientTasks amClientTasks) {
        try {
            String tasksData = amClientTasks.getData();
            String bossId = amZpLocalAccouts.getId();
...
//            return ResultVO.fail(404, "找不到筛选条件");
        ...
        StringBuilder filterReason = new StringBuilder();
        Boolean result = innerFilterAmResume(conditionNewServiceOne, amResume,isGreet,filterReason);
    ...
    public ResultVO filterAndSaveAmResume(String platform,String bossId, String connectId, String encryptId,JSONObject resumeObject,String greet_task_id) {
        ...
        AmResume amResume = buildAmResume(amPositionServiceOne, amZpLocalAccouts, resumeObject);
        StringBuilder filterReason = new StringBuilder();
        Boolean result = innerFilterAmResume(conditionNewServiceOne, amResume,true,filterReason);
...
//                    amChatbotGreetResult.setTaskId(Integer.parseInt(greetId));
    ...
    public Boolean innerFilterAmResume(AmChatbotGreetConditionNew conditionNewServiceOne,AmResume amResume,Boolean isGreet,StringBuilder filterReason) {
        AmGreetConditionVo amGreetConditionVo = AmChatBotGreetNewConditionConvert.I.convertGreetConditionVo(conditionNewServiceOne);
        boolean result = AmResumeFilterUtil.filterResume(amResume, amGreetConditionVo,isGreet,filterReason);
