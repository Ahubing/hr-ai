...
public class AiReplyPromptUtil {
    ...
    public static final String baseInfoPrompt = "# 回复格式\n" +
            ...
            "{\"messages\":[\"你的简历符合我们的招聘要求\", \"我们可以约一个时间面试\", \"你看一下这周什么时间方便？\"], \"elimination_reason\":\"xxx\"}\n" +
            "参数解释: \n" +
            "messages是发送给候选人的消息，你可以模仿人类发消息时往往分成几段来发送。你可以让messages为空数组，代表不发送消息。\n" +
            "elimination_reason 是回复候选人“不符合”的原因，仅在你认为候选人不符合要求时填写\n\n";
    ...
    public static final String interviewPrompt = "# 面试信息\n" +
            ...
            "如果面试方式不存在，则不回复预约面试的消息。\n" +
            "如果是在线面试，面试链接将在面试前由系统自动发送。\n" +
            "- 面试方式：{address} \n";
    ...
    public static final String processControlPrompt = "\n" +
            "# 沟通进度\n" +
            "沟通进度共有以下几种：\n" +
            "-1. 不符合（AI控制进入）：无意向或已被淘汰\n" +
            "1. 业务筛选（AI控制进入）：进行意向确认，求职者有意向则使用“求职者筛选提示词”对求职者进行筛选（为空则不需要进行筛选），筛选通过后进入下一状态。无意向或未通过筛选进入“不符合”状态\n" +
            "2. 邀约跟进（AI控制进入）：预约面试（时间，地点），并与求职者确认，求职者确认后调用函数预约面试时间\n" +
            "3. 等待面试（系统控制进入）：调用函数预约面试成功后向求职者发送完整面试信息，并提示按时参加\n" +
            "4. 已发offer（系统控制进入）\n" +
            "5. 已入职（系统控制进入）\n" ;
