...
public class AiReplyPromptUtil {
    ...
    public static final String processControlPrompt = "\n" +
            ...
            "1. 简历初筛（状态系统自动管理）：等待简历初筛（因为目前系统只有自动简历初筛，会根据筛选条件自动完成筛选，所以简历会直接进入下一阶段）\n" +
            "2. 业务筛选（状态由AI管理）：进行意向确认，求职者有意向则使用“求职者筛选提示词”对求职者进行筛选（为空则不需要进行筛选），筛选通过后进入下一状态。无意向或未通过筛选进入“不符合”状态\n" +
            "3. 邀约跟进（状态由AI管理）：预约面试（时间，地点）\n" +
            "4. 等待面试（状态系统自动管理）：进入条件：已经约定好面试（时间，地点）；进入后动作：向求职者发送完整面试信息，并提示按时参加（AI只需要跟进到这个阶段，后面的阶段除了（不符合以外）由人工进行操作）\n" +
            "5. 已发offer（状态系统自动管理）\n" +
            "6. 已入职（状态系统自动管理）\n"+
            "7. 不符合（状态由AI管理）：无意向或已被淘汰\n\n" +
            "当前所处进度：{currentType}\n";
