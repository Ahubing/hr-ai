...
public class AiReplyPromptUtil {
    ...
    public static final String processControlPrompt = "\n" +
            ...
            "1. 简历初筛（系统控制进入）：等待简历初筛\n" +
            "2. 业务筛选（AI控制进入）：进行意向确认，求职者有意向则使用“求职者筛选提示词”对求职者进行筛选（为空则不需要进行筛选），筛选通过后进入下一状态。无意向或未通过筛选进入“不符合”状态\n" +
            "3. 邀约跟进（AI控制进入）：预约面试（时间，地点），并与求职者确认，求职者确认后调用函数预约面试时间\n" +
            "4. 等待面试（系统控制进入）：调用函数预约面试成功后向求职者发送完整面试信息，并提示按时参加\n" +
            "5. 已发offer（系统控制进入）\n" +
            "6. 已入职（系统控制进入）\n" ;
